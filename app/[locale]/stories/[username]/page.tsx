...
export async function generateMetadata({
  ...
  const localizedExperienceSummary = getLocalizedField(
    user?.experienceSummary || null,
    user?.experienceSummaryTranslation || null,
    locale
  );
  
  return {
    title: "Reddit user /u/" + username + "'s Detransition Story | detrans.ai",
    description: markdownToPlainText(localizedExperienceSummary?.slice(0, 300)),
    openGraph: {
...
export default async function UserPage({
  ...
  const localizedExperience = getLocalizedField(
    user.experience,
    user.experienceTranslation,
    locale
  );
  
  const localizedRedFlagsReport = getLocalizedField(
    user.redFlagsReport,
    user.redFlagsReportTranslation,
    locale
  );
  ...
  return (
    <div className="container mx-auto px-0 pb-8">
      ...
      <div className="prose dark:prose-invert max-w-full space-y-6">
        ...
        {/* Red Flags Report */}
        {localizedRedFlagsReport && (
          <details className="cursor-pointer text-sm sm:text-base">
            <summary className="text-muted-foreground">
              <i>
                Authenticity Assessment:{" "}
                {user.tags.some((tag) => tag.name === "suspicious account")
                  ? "Suspicious Account"
                  : "Not Suspicious"}
              </i>
            </summary>
            <div
              dangerouslySetInnerHTML={{
                __html: marked.parse(localizedRedFlagsReport || ""),
              }}
          ...
          <div className="">
            ...
            <div className="border-t">
              <div
                ...
                dangerouslySetInnerHTML={{
                  __html: marked.parse(localizedExperience || ""),
                }}
              ></div>
            </div>
          </div>
