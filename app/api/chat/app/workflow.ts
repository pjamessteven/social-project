...
export const workflowFactory = async (reqBody: any) => {
  console.log('[WORKFLOW] Starting workflow factory with reqBody:', !!reqBody);
  
  initSettings();
  
  console.log('[WORKFLOW] Getting indexes...');
  ...
  console.log('[WORKFLOW] Tools created:', {
    commentsToolName: commentsEngineTool.metadata?.name || 'unnamed',
    storiesToolName: storiesEngineTool.metadata?.name || 'unnamed'
  });

  const workflow = agent({ 
    tools: [commentsEngineTool, storiesEngineTool],     
  ...
  console.log('[WORKFLOW] Workflow created successfully');
  return workflow;
};
